...
class OwnershipAccessTest {
    ...
    void alunoNaoPodeAcessarOutroAluno() {
        ...
        Aluno alunoAutenticado = aluno(alunoAutenticadoId, "Aluno Autenticado", "autenticado@lab03.com");
        when(currentUserService.getAuthenticatedAluno()).thenReturn(alunoAutenticado);
        when(alunoRepository.findById(outroAlunoId)).thenReturn(Optional.of(aluno(outroAlunoId, "Outro", "outro@lab03.com")));
    ...
    void professorNaoPodeAcessarOutroProfessor() {
        ...
        Professor professorAutenticado = professor(professorAutenticadoId, "Professor Autenticado", "prof@lab03.com");
        when(currentUserService.getAuthenticatedProfessor()).thenReturn(professorAutenticado);
        when(professorRepository.findById(outroProfessorId)).thenReturn(Optional.of(professor(outroProfessorId, "Outro Prof", "outroprof@lab03.com")));
    ...
    void empresaNaoPodeAcessarOutraEmpresa() {
        ...
        when(currentUserService.getAuthenticatedEmpresa()).thenReturn(empresaAutenticada);
        when(empresaParceiraRepository.findById(outraEmpresaId)).thenReturn(Optional.of(empresa(outraEmpresaId, "Outra Empresa", "outra@lab03.com")));
