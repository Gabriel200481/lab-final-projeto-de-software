...
/**
 * ServiÃ§o de geraÃ§Ã£o de QR Code para cupons de resgate
 * 
 * Responsabilidades:
 * - Gerar QR Code em formato PNG
 * - Persistir arquivo em disco/storage
 * - Retornar URL pÃºblica do QR Code
 * - Tratamento de erros
 */
@Singleton
public class QrCodeService {
     ...
     * 
     * @param dados ConteÃºdo do QR Code (ex: "ABC-12345-XYZ")
     * @param nomeArquivo Nome do arquivo a salvar (sem extensÃ£o)
     * @return URL pÃºblica do QR Code gerado
     * @throws Exception Se houver erro na geraÃ§Ã£o
     */
    public String gerarQrCode(String dados, String nomeArquivo) throws Exception {
        logger.info("Iniciando geraÃ§Ã£o de QR Code para: {}", nomeArquivo);
        
        try {
            // 1. Criar diretÃ³rio se nÃ£o existir
            Path dirPath = Paths.get(qrcodePath);
            Files.createDirectories(dirPath);
            logger.debug("DiretÃ³rio QR Code verificado: {}", qrcodePath);
            ...
            logger.info("URL pÃºblica gerada: {}", urlPublica);
    ...
    /**
     * Gera nome de arquivo Ãºnico para QR Code
     * 
     * @param usuarioId ID do usuÃ¡rio
     * @param vantagemId ID da vantagem
     * @return Nome Ãºnico para arquivo
     */
     ...
     * @return true se arquivo existe e Ã© vÃ¡lido
     */
    public boolean validarQrCode(String urlQrCode) {
        try {
            ...
            if (!arquivo.exists()) {
                logger.warn("Arquivo QR Code nÃ£o encontrado: {}", nomeArquivo);
                return false;
            }
